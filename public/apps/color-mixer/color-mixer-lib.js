 ...
 * contrastRatio / pickTextColor 與 faber-castell-color、caran-dache-color、
 * copic-color、finecolour-color、enmy-color 五支 lib **逐字相同**——
 * 家族的比對器必須用同一把尺，否則「最接近的筆」在不同 app 會給出不同答案。
 * （這一段五支＋本支共六份複製，抽成共用件 `color-metric.js` 是對的，
 *   但那是一次跨 6 個 repo 的收斂，刻意不混進本 app 的第一版。）
 *
 * ── 為什麼沒有 nearestAcross ──────────────────────────────────────────
...
(function (global) {
  ...
  // ---- 色彩換算（與五支色彩 registry 的 lib 逐字相同） --------------------
